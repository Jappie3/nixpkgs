    ...
    assertions = [
      {
        ...
        message = ''
          The options <option>services.spliit.settings.POSTGRES_PRISMA_URL</option> and
          <option>services.spliit.settings.POSTGRES_URL_NON_POOLING</option> should not
          be set when <option>services.spliit.database.createLocally</option> is set to false
        '';
      }
    ];
